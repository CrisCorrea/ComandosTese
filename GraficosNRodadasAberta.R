GraficosNRodadasAberta <- function (i,DadosServidores,MatrizDeEstados,tempo){ #faz gráficos para várias rodadas
...
for (k in 1:rodadas) for (j in 1:pop) for (t in 1:tempo){    
      ...
      if (t<=TE[j,k]) Estado[j,t,k]=0    ##Linha necessária apra pop aberta
}
...
#    png(paste("VariaçãoEstados",j," ", pop, "serv. Pop",i,".png"))
...
#    png(paste("DiferençaEstados",j," ", pop, "serv. Pop",i,".png"))
...
plot(Maximo[1,], type="l" , lty =2, ylab="Frequência", xlab="Tempo",main=paste("Painel A -",(estados[1])),cex.axis=1.5,cex.lab=1.5, cex=1.5, cex.main=1.5,lwd=2)
lines(Minimo[1,],lty =3, cex=1.5,lwd=2)
lines(MatrizMedia[1,],col="red", cex=1.5,lwd=2)
legend("topleft",legend=c("Máximo","Média","Mínimo"), col=c("black","red","black"), lty = c(2,1,3), cex=1.5, bty="n")    
dev.off()
png(paste("Extremos",2,"Aberta ", pop, "serv. Pop",i,".png"))    
plot(Maximo[2,], type="l" , lty =2, ylab="Frequência", xlab="Tempo",main=paste("Painel C -",(estados[2])),cex.axis=1.5,cex.lab=1.5, cex=1.5, cex.main=1.5,lwd=2)
...
png(paste("Desvio Padrão dos estados Aberta", pop, "serv. Pop",i,".png"))
...
dev.off()
write.table(DP, paste("DESVIO PADRÃO DOS ESTADOS", pop, "serv.txt"), sep="\t",dec=",")

png(paste("Diferença Máxima Aberta", pop, "serv. Pop",i,".png"))
...
legend ("topright",legend=estados[4:6],lty = seq(4:6),lwd=2,col = c("dark orange","brown","black"), cex=1.2, bty="n")
dev.off()
write.table(dif,paste("Diferença máxima no estado", pop, "serv.txt"), sep="\t",dec=",")

png(paste("Média Aberta", pop, "serv.png")) 
...
png(paste("Tempo médio de contribuição e benefício Aberta- ", pop, "serv. Pop",i,".png"))
